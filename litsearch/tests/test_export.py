...
def test_braces_in_a_title_cannot_unbalance_the_entry():
    nasty = Work(
        title="A title with {unbalanced braces and a \\command", doi="10.1/x", year="2020", authors=["Ann Author"]
    )
...
def test_unicode_punctuation_is_folded_to_ascii():
    ...
    work = Work(
        title="Qubit\u2010based \u201cdesign\u201d \u2013 an approach\u2026",
        doi="10.1/x",
        year="2020",
        authors=["Ann Author"],
    )
...
def test_preprints_get_no_volume_or_pages():
    work = Work(
        title="A preprint", arxiv_id="2101.00001", year="2021", authors=["Ann Author"], volume="12", pages="1--2"
    )
    entry = loads(build_bibtex([work])).entries[0]
...
def test_all_non_latin_authors_still_produce_an_ascii_key():
    work = Work(
        title="A paper", doi="10.1/cyr2", year="2005", authors=["\u0421. \u041f. \u0413\u0443\u0431\u0438\u043d"]
    )
    entry = loads(build_bibtex([work])).entries[0]
...
def test_evidence_columns_follow_the_search_schema(tmp_path):
    ...
    rows = [
        {
            "cite_key": "Chapman2023",
            "gate_type": "beam splitter",
            "coupler": "SNAIL",
            "fidelity_pct": 99.9,
            "source_quote": "a 50:50 beamsplitter in 125 ns",
